(async()=>{
  ...
  try{
    ...
    if(p.service_scope?.length){
      deep+=section('SERVICE SCOPE','보이지 않는 곳까지 이어지는 전담 조율',`<ol class="numbered-editorial">${p.service_scope.map((x,i)=>`<li><span>${String(i+1).padStart(2,'0')}</span><p>${esc(x)}</p></li>`).join('')}</ol>`);
    }
    if(p.phases?.length){
      deep+=section('PRIVATE JOURNEY','과정마다 필요한 역할을 분리해 조율합니다.',`<div class="phase-editorial">${p.phases.map((ph,i)=>`<article><span>${String(i+1).padStart(2,'0')}</span><small>${esc(ph.name)}</small><h3>${esc(ph.ko)}</h3>${list(ph.items,'phase-items')}</article>`).join('')}</div>`);
    }
    if(p.medical_fields?.length){
      deep+=section('SELECTED FIELDS','상담 목적에 따라 필요한 영역만 선별합니다.',`<div class="field-index">${p.medical_fields.map((x,i)=>`<div><span>${String(i+1).padStart(2,'0')}</span><strong>${esc(x)}</strong></div>`).join('')}</div>`);
    }
    if(p.tiers?.length){
      deep+=section('PRIVATE SERVICE LEVELS','범위는 고객의 목적과 일정에 따라 달라집니다.',`<div class="tier-editorial">${p.tiers.map(t=>`<article><small>${esc(t.name)}</small><h3>${esc(t.copy)}</h3><p>${esc(t.fit)}</p></article>`).join('')}</div>`);
    }
    if(p.specialized_journeys?.length){
      deep+=section('SELECTED JOURNEYS','의료 목적에 따라 별도의 여정으로 확장할 수 있습니다.',`<div class="journey-index">${p.specialized_journeys.map((x,i)=>`<div><span>${String(i+1).padStart(2,'0')}</span><strong>${esc(x)}</strong></div>`).join('')}</div>`);
    }
    ...
    root.innerHTML=`
      ...
      <section class="detail-body detail-body-lux">
        <div class="detail-columns detail-columns-lux">
          <div class="detail-intro detail-intro-lux">
            <p class="kicker gold">PRIVATE PROGRAM DESIGN</p>
            <h2>${esc(p.ko)}</h2>
            <p>at PRIVÉ는 정해진 패키지를 일괄적으로 제안하지 않습니다. 고객의 목적과 기존 정보, 일정과 프라이버시를 먼저 확인하고 필요한 의료 접근과 컨시어지 범위를 조율합니다.</p>
      ...
      <section class="closing-notice">
        <div><p>MEDICAL NOTICE</p><h2>의학적 판단은 의료진이 직접 진행합니다.</h2></div>
